...
def conv_layer(hidden_layer_output, name):
    h0 = tf.nn.sigmoid(conv1d(hidden_layer_output, FILTERS[0], stride=1, name=name+'_h0_conv'))
    h1 = tf.nn.sigmoid(conv1d(h0, FILTERS[1], stride=2, name=name+'_h1_conv'))
    h2 = tf.nn.sigmoid(conv1d(h1, FILTERS[2], stride=1, name=name+'_h2_conv'))
